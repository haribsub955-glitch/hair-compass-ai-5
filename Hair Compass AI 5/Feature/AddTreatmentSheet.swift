...
struct AddTreatmentSheet: View {
    ...
    @State private var analysisService = HairAnalysisService()
    /// Bumped when the consent card records a choice — `CloudAIConsent` lives in UserDefaults,
    /// which SwiftUI cannot observe, so this is what re-evaluates `analysisService.engine`.
    ...
    /// The label reading and the written summary are honestly different privacy stories: OCR is
    /// always on-device Vision (the photo never leaves this iPhone), while the summary follows
    /// `AIEngine` — the recognized *text* goes to the cloud model once consented, or to Apple
    /// Intelligence otherwise. The footnote under the button says which one applies right now.
    private var ingredientFootnote: String {
    ...
    private func analyzeIngredientsTapped() {
        ...
        // The photo was already read by on-device Vision OCR (loadIngredientPhoto); the summary
        // engine — cloud once consented, on-device otherwise — is decided in HairAnalysisService.
