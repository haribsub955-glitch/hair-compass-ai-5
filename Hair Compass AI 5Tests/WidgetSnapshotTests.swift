...
struct WidgetSnapshotTests {
    ...
    // MARK: - Entitlement-aware suppression (Task 8: the widget is a second read path)

    /// The widget must not backdate its "latest" reading past what a free tier can review
    /// in-app — `entries.max` used to reach straight past `HistoryAccess`, so someone who
    /// skipped today would still see a days-old shed/scalp reading on the Home Screen even
    /// though `LockedHistoryCard` walls that same entry off inside the app.
    @Test @MainActor func freeTierGetsNoShedOrScalpLabelFromAnUnloggedDay() {
        let cal = Calendar.current
        let now = Date.now
        let yesterday = cal.date(byAdding: .day, value: -1, to: now)!
        let staleEntry = DailyEntry(date: yesterday, shed: .elevated, flaking: 1, erythema: 1, itch: 1)

        let snap = WidgetSnapshotBuilder.build(
            entries: [staleEntry], treatments: [], doses: [], photos: [],
            entitlements: Entitlements(tier: .free), now: now, calendar: cal
        )
    ...
    /// Pro sees exactly the same "latest" reading as before — the filter is a no-op once
    /// `HistoryAccess.canAccess(.history)` is true.
    @Test @MainActor func proTierStillGetsTheLatestShedAndScalpLabel() {
        let cal = Calendar.current
        let now = Date.now
        let yesterday = cal.date(byAdding: .day, value: -1, to: now)!
        let staleEntry = DailyEntry(date: yesterday, shed: .elevated, flaking: 1, erythema: 1, itch: 1)
        ...
        let snap = WidgetSnapshotBuilder.build(
            entries: [staleEntry], treatments: [], doses: [], photos: [],
            entitlements: Entitlements(tier: .pro), now: now, calendar: cal
        )

        #expect(snap.shedLabel == ShedLevel.elevated.title)
        #expect(snap.scalpLabel == "Scalp \(staleEntry.scalpBand.title.lowercased())")
    }

    /// `.treatments` and `.photos` are both Pro-gated in-app, so a free tier's widget must not
    /// spell out treatment names or carry photo-completion state either — the Care/Lens rings
    /// fall back to their existing "nothing to show" states instead of a new one.
    @Test @MainActor func freeTierWithdrawsTreatmentDetailAndPhotoSignal() {
        let now = Date.now
        let minoxidil = Treatment(
            name: "Minoxidil", treatmentClass: .minoxidil,
            scheduleTimes: "08:00,21:00", startDate: now, isActive: true
        ...
        let snap = WidgetSnapshotBuilder.build(
            entries: [], treatments: [minoxidil], doses: [], photos: [photo],
            entitlements: Entitlements(tier: .free), now: now
        )

        #expect(snap.dueTitles.isEmpty)
        #expect(snap.ringCare == nil)
        #expect(snap.ringLens == 0)
    }

    /// The streak is the one thing that must NOT shrink to match the filtered history — free
    /// users log forever and the counter keeps climbing even though they can't review the days
    /// behind it (HistoryAccess.swift). Filtering the entries fed to `shieldedStreak` the same
    /// way as `shedLabel`/`scalpLabel` would silently wrong the number instead of just hiding it.
    @Test @MainActor func freeTierStreakStillReflectsTheFullUnfilteredHistory() {
        let cal = Calendar.current
        let now = Date.now
        let entries = (0..<7).compactMap { offset in
            cal.date(byAdding: .day, value: -offset, to: now).map { DailyEntry(date: $0) }
        ...
        let snap = WidgetSnapshotBuilder.build(
            entries: entries, treatments: [], doses: [], photos: [],
            entitlements: Entitlements(tier: .free), now: now, calendar: cal
        )
