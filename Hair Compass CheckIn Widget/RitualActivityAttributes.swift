...
//   Model/RitualActivityAttributes.swift                          (app target)
//   Hair Compass CheckIn Widget/RitualActivityAttributes.swift    (widget extension target, this file)
...
/// (Feature/Ritual/RitualView.swift, app target) is on screen. Launch rituals are short,
/// full-screen mini games — today each is a single continuous phase, not a multi-page sequence,
/// so `stepIndex`/`totalSteps` are fixed at 1/1; they exist so a future multi-phase ritual
/// doesn't need an ActivityAttributes migration.
